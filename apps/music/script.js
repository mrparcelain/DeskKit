/* ==========================================================================
   Music app
   --------------------------------------------------------------------------
   Runs once, the first time the Music window opens. Everything is scoped to
   this function, so names here cannot clash with other apps.

   Data:   apps/music/music.json (edit it with /developer → Music)
   Assets: apps/music/assets/
   ========================================================================== */

Desktop.app('music', ({ url, onModeChange }) => {
  ...
  const LOCAL_LIBRARY_KEY = 'deskkit.music.draft';
  ...
  // switching to the phone layout (window made narrow) pauses desktop playback
  onModeChange((mode) => { if (mode === 'mobile' && currentAudio) currentAudio.pause(); });
  ...
  repeatButton.addEventListener('click', () => {
    ...
    if (isRepeating) {
      repeatButton.src = url('assets/repeat-on.svg');
    } else {
      repeatButton.src = url('assets/repeat-off.svg');
    }
  });
